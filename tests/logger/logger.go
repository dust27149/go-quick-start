...
var rotatingWriter *writer.RotatingWriter

// Init 初始化日志系统，设置日志输出目录和文件名。
func Init(logConfig config.LogConfig) error {
	// 初始化日志目录和文件
	rotatingWriter, err := writer.InitWriter(logConfig)
	if err != nil {
		return err
	}

	// 输出到 RotatingWriter 本身，确保写入时触发大小检查与轮转压缩。
	logger = log.New(io.MultiWriter(os.Stdout, rotatingWriter), "", log.Ldate|log.Ltime|log.Lshortfile)
